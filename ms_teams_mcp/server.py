...
def read_channel_file(drive_id: str, item_id: str) -> str:
    ...
    # Content extraction (xlsx / hwp / text) is shared with the attachment
    # readers via _extract_file_content so all file handling stays in sync.
    return f"Filename: {name}\n{'─'*50}\n{_extract_file_content(name, resp.content)}"


# ─────────────────────────────────────────
# Attachment & generic resource readers
# (email / chat / channel attachments + Graph read_resource, M365-style)
# ─────────────────────────────────────────

def _extract_file_content(name: str, content: bytes, max_chars: int = 8000) -> str:
    """Extract readable text from raw file bytes, dispatching on extension.

    Shared by read_channel_file and the attachment readers so Excel / HWP /
    plain-text handling lives in exactly one place. `name` is used only to
    select the parser by file extension.
    """
    lname = (name or "").lower()
    ...
    if lname.endswith(".hwp"):
        return _extract_hwp_text(content)[:max_chars]
    ...
    try:
        ...
        try:
            ...
            return (f"[Binary file '{name}' - text extraction not supported. "
                    f"Size: {len(content)} bytes.]")
    return text[:max_chars]


def _encode_share_url(url: str) -> str:
    """Encode a SharePoint/OneDrive sharing URL into a Graph shareId (u!...)."""
    import base64
    b64 = base64.urlsafe_b64encode(url.encode("utf-8")).decode("ascii").rstrip("=")
    return "u!" + b64


def _download_shared_drive_item(content_url: str, max_bytes: int = 5 * 1024 * 1024):
    """Resolve a chat/channel attachment contentUrl (a SharePoint/OneDrive
    sharing or web URL) to a driveItem via the Graph /shares endpoint, then
    download the bytes. Returns (name, content_bytes, mime_type)."""
    share_id = _encode_share_url(content_url)
    meta = graph_get(f"/shares/{share_id}/driveItem")
    name = meta.get("name", "")
    size = meta.get("size", 0)
    mime = meta.get("file", {}).get("mimeType", "")
    if size and size > max_bytes:
        raise Exception(f"File too large ({size / 1024 / 1024:.1f}MB > "
                        f"{max_bytes / 1024 / 1024:.0f}MB cap).")
    download_url = meta.get("@microsoft.graph.downloadUrl", "")
    if download_url:
        resp = requests.get(download_url)
    else:
...
def _resolve_teams_message(message_id: str, chat_id: str = "",
                           team_id: str = "", channel_id: str = "") -> dict:
    """Fetch a single Teams chat or channel message (including its attachments)."""
    if chat_id:
        try:
            return graph_get(f"/me/chats/{chat_id}/messages/{message_id}")
        except Exception:
            return graph_get(f"/chats/{chat_id}/messages/{message_id}")
    if team_id and channel_id:
        return graph_get(
            f"/teams/{team_id}/channels/{channel_id}/messages/{message_id}"
        )
    raise Exception("Provide chat_id, or both team_id and channel_id.")
...
def list_email_attachments(message_id: str) -> str:
    ...
    - message_id: ID from list_emails / search_emails
    Use read_email_attachment with the returned id to extract a file's text.
    """
    data = graph_get(
    ...
    for i, a in enumerate(atts, 1):
        atype = a.get("@odata.type", "").split(".")[-1] or "attachment"
        inline = " (inline)" if a.get("isInline") else ""
        lines.append(
            f"{i}. {a.get('name') or '(no name)'}{inline}\n"
            f"   type: {atype}  contentType: {a.get('contentType','')}  "
...
@mcp.tool()
def read_email_attachment(message_id: str, attachment_id: str,
                          max_chars: int = 8000) -> str:
    """
    Read and extract text from a single email attachment (xlsx, hwp, csv, txt, ...).
    - message_id: email ID
    - attachment_id: id from list_email_attachments
    Image / audio / video and other binary attachments cannot be extracted to text.
    """
    ...
    if "itemAttachment" in atype:
        return (f"'{name}' is an attached Outlook item (email/event), not a file. "
                f"Item attachments are not extracted.")

    if "referenceAttachment" in atype:
        link = a.get("sourceUrl") or a.get("contentUrl") or ""
        if not link:
            return f"'{name}' is a reference attachment with no resolvable URL."
        try:
            fname, content, _mime = _download_shared_drive_item(link)
            return (f"Filename: {fname or name} (reference)\n{'─'*50}\n"
                    f"{_extract_file_content(fname or name, content, max_chars)}")
        except Exception as e:
            return (f"'{name}' is a reference attachment; could not fetch "
                    f"content: {e}\nLink: {link}")

    size = a.get("size", 0)
    if size and size > 5 * 1024 * 1024:
        return f"Attachment too large ({size / 1024 / 1024:.1f}MB). Cap is 5MB."
    ...
    return (f"Filename: {name}\n{'─'*50}\n"
            f"{_extract_file_content(name, content, max_chars)}")


@mcp.tool()
def list_message_attachments(message_id: str, chat_id: str = "",
                             team_id: str = "", channel_id: str = "") -> str:
    """
    List file attachments on a Teams chat or channel message.
    Provide message_id plus EITHER chat_id (1:1/group chat) OR team_id+channel_id (channel).
    Use read_message_attachment with the returned contentUrl (or id) to extract a file's text.
    """
    ...
    for i, a in enumerate(atts, 1):
        lines.append(
            f"{i}. {a.get('name') or '(no name)'}\n"
            f"   contentType: {a.get('contentType','')}\n"
            f"   id: {a.get('id','')}\n"
            f"   contentUrl: {a.get('contentUrl','')}"
        )
    return "\n".join(lines)


@mcp.tool()
def read_message_attachment(message_id: str = "", attachment_id: str = "",
                            content_url: str = "", chat_id: str = "",
                            team_id: str = "", channel_id: str = "",
                            max_chars: int = 8000) -> str:
    """
    Read and extract text from a file attached to a Teams chat or channel message.
    Two ways to call:
      1) Pass content_url directly (from list_message_attachments).
      2) Pass message_id + attachment_id + scope (chat_id, OR team_id+channel_id).
    """
    url = content_url
    name_hint = ""
    if not url:
    ...
    try:
        fname, content, mime = _download_shared_drive_item(url)
    except Exception as e:
        return f"Could not download attachment: {e}\nURL: {url}"
    fname = fname or name_hint
    if any(t in mime for t in ("image", "video", "audio")):
        return (f"Filename: {fname}\nBinary media ({mime}); text extraction "
...
def read_resource(uri: str) -> str:
    """
    Read any Microsoft Graph resource by path or full URL (read-only GET),
    mirroring Microsoft 365's read_resource. Returns the JSON response as text.
    - uri: e.g. '/me/messages/{id}/attachments',
           '/teams/{id}/channels/{id}/messages/{id}',
           or a full 'https://graph.microsoft.com/...' URL.
    For attachment *content*, prefer read_email_attachment / read_message_attachment.
    """
    import json as _json
    if uri.startswith("http"):
        data = graph_get("", url=uri)
    else:
        if not uri.startswith("/"):
            uri = "/" + uri
        data = graph_get(uri)
    return _json.dumps(data, ensure_ascii=False, indent=2)[:12000]
